...
3. Fetch Open-Meteo archive weather one location at a time for `2022-06-01..2026-05-31`, the latest complete overlap currently used by P0056C/P0056B evidence.
4. Upsert location-hour observations and weight rows.
5. Build zone hourly means and weighted area proxy/features rows.
6. Load P0056A targets, join with P0056D weather features, and reuse P0056C model helper functions for split, fitting and metric calculation.
7. Persist P0056D forecast logs/metrics for SE1, SE2 and FI only.
8. Compare metrics against P0056C baseline evidence and write required evidence files.
...
- Open-Meteo availability can lag; missing hours must be documented.
