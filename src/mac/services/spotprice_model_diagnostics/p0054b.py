"""P0054B LABB SE3 consumption advanced AI without price."""
...
PACKAGE_ID = "P0054B"
LABEL = "LABB"
EVIDENCE_DIR = Path("requirements/package-runs/P0054B")
SOURCE_TABLE = "physical_balance_se1_se4_hourly_v1"
WEATHER_TABLE = "weather_proxy_se3_load_hourly"
...
def run_p0054b_analysis(
    *,
    ...
    if not target_contract["ok"]:
        raise RuntimeError(f"P0054B target contract failed: {target_contract}")
    ...
    if not feature_review["ok"]:
        raise RuntimeError(f"P0054B forbidden feature contract: {feature_review}")
    ...
    evidence = write_p0054b_evidence(Path(evidence_dir), scored_rows, scored_path_rows, weekly_path_rows, summary)
    return P0054BResult(status=str(summary["status"]), row_counts=summary["row_counts"], evidence=evidence)


def load_se3_consumption_rows(feature_db: Path | str = DEFAULT_FEATURE_DB) -> list[dict[str, object]]:
    with sqlite3.connect(Path(feature_db).expanduser()) as conn:
        conn.row_factory = sqlite3.Row
        if not table_exists(conn, SOURCE_TABLE):
            raise RuntimeError(f"P0054B source table missing: {SOURCE_TABLE}")
        return [
...
def rolling_features_at_origin(values: list[float], origin_index: int) -> dict[str, float]:
    ...
    for window in ROLL_WINDOWS:
        ...
        out[f"consumption_se3_roll_mean_{window}h"] = sum(subset) / len(subset)
...
def apply_profile_features(rows: list[dict[str, object]], profiles: dict[str, object]) -> None:
    for row in rows:
        ...
        if is_finite(row.get("weather_proxy_temperature_2m_se3")):
            row["weather_proxy_temperature_delta_from_train_normal_se3"] = (
                float(row["weather_proxy_temperature_2m_se3"]) - float(row["weather_proxy_train_normal_temperature_2m_se3"])
            )
        else:
            row["weather_proxy_temperature_delta_from_train_normal_se3"] = 0.0
...
def apply_baseline_predictions(rows: list[dict[str, object]], profiles: dict[str, object], source_by_ts: dict[str, dict[str, object]]) -> None:
    ...
    for row in rows:
        ...
        row["pred_B4_recent_24h_mean_adjusted_by_hour_profile"] = float(row["consumption_se3_roll_mean_24h"]) + profile_delta
...
def forecast_safe_history_lookup(
    ...
    if desired_dt < origin_dt:
        ...
        if source:
            return float(source["consumption_se3"])
    ...
    for offset in range(24, 24 * 28 + 1, 24):
        ...
        if source and int(source["model_cet_hour"]) == target_hour:
            return float(source["consumption_se3"])
    return float(row["consumption_se3_roll_mean_168h"])
...
def evaluate_weekly_168h_paths(scored_rows: list[dict[str, object]], prediction_columns: tuple[str, ...]) -> tuple[dict[str, object], list[dict[str, object]]]:
    ...
    for prediction_column in prediction_columns:
        summary[prediction_column] = regression_metric_from_predictions(
            [row for row in rows if row.get(prediction_column) is not None],
            [float(row[prediction_column]) for row in rows if row.get(prediction_column) is not None],
        )
...
def evaluate_conditional_regimes(rows: list[dict[str, object]], prediction_columns: tuple[str, ...]) -> dict[str, object]:
    ...
    deltas = [safe_float(row.get("weather_proxy_temperature_delta_from_train_normal_se3")) for row in scored if is_finite(row.get("weather_proxy_temperature_delta_from_train_normal_se3"))]
    ...
    regimes = {
        "cold_hours_temperature_p25_or_lower": lambda row: safe_float(row.get("weather_proxy_temperature_2m_se3")) <= temp_p25,
        "very_cold_hours_temperature_p10_or_lower": lambda row: safe_float(row.get("weather_proxy_temperature_2m_se3")) <= temp_p10,
        "rapid_temperature_drop_proxy_p10_or_lower": lambda row: safe_float(row.get("weather_proxy_temperature_delta_from_train_normal_se3")) <= delta_p10,
...
def write_p0054b_evidence(
    ...
    files["scored_rows_csv"] = write_csv(
        evidence_dir / "p0054b_scored_direct_rows_sample.csv",
        rows[:5000],
...
def evidence_markdowns(summary: dict[str, object]) -> dict[str, str]:
    ...
    return {
        "CHANGELOG.md": common_header + "Built an offline SE3 consumption LABB experiment with HGB benchmark and small MLP advanced AI. No API, devices, runtime writes, price, production, flow, export/import or A61 inputs were used.\n",
        ...
        "baseline-results.md": common_header + json_block(summary["baseline_results"]),
        ...
        "component-attribution-summary.md": common_header + "Component attribution is limited to feature groups, baselines and HGB light permutation importance. The LABB run isolates calendar, historical SE3 load state and weather proxy. Price and grid components are intentionally absent.\n",
        "interpretation.md": common_header
        + f"HGB holdout MAE: {hgb_holdout['MAE']:.3f} MW. MLP holdout MAE: {mlp_holdout['MAE']:.3f} MW. Holdout winner by MAE: {comparison['holdout_winner_by_MAE']}. Weather-assisted numbers are LABB proxy results because realized archive weather is not a separate forecast model.\n",
        "what-we-learned.md": common_header
        + "P0054B measures the non-price SE3 consumption forecasting ceiling available from calendar, historical load state and weather proxy. The result should guide whether later spread/flaskhals-regime packages need consumption residuals, but it must not be promoted directly.\n",
        "next-package-recommendation.md": common_header
        + "Recommended next package remains an advanced AI LABB for SE3-SE1 spread and bottleneck regimes, using P0054A LABB governance and keeping forecast_safe/proxy/oracle inputs separated.\n",
    }
...
def main() -> None:
    args = parse_args()
    result = run_p0054b_analysis(feature_db=args.feature_db, weather_db=args.weather_db, evidence_dir=args.evidence_dir)
