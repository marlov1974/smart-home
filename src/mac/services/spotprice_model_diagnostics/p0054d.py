"""P0054D LABB SE4 consumption advanced AI without price."""
...
PACKAGE_ID = "P0054D"
LABEL = "LABB"
EVIDENCE_DIR = Path("requirements/package-runs/P0054D")
SOURCE_TABLE = "physical_balance_se1_se4_hourly_v1"
WEATHER_TABLE = "weather_area_hourly"
WEATHER_AREA_PROXY = "se4_load_weather"
...
EXTRATREES_PREDICTION_COLUMN = "pred_ExtraTrees_G4_calendar_load_lags_rollups_weather_proxy"
MODEL_PREDICTION_COLUMNS = (HGB_PREDICTION_COLUMN, MLP_PREDICTION_COLUMN, EXTRATREES_PREDICTION_COLUMN)
P0054C_SUMMARY_PATH = Path("requirements/package-runs/P0054C/summary.json")
...
def run_p0054d_analysis(
    *,
    ...
    if not target_contract["ok"]:
        raise RuntimeError(f"P0054D target contract failed: {target_contract}")
    ...
    if not feature_review["ok"]:
        raise RuntimeError(f"P0054D forbidden feature contract: {feature_review}")
    ...
    extra_result = fit_extra_trees(rows, feature_contract["G4_calendar_load_lags_rollups_weather_proxy"]["features"])
    scored_rows = attach_model_predictions(rows, hgb_result["predictions"], mlp_result["predictions"], extra_result["predictions"])
    ...
    p0054c_summary = load_p0054c_summary()
    model_results = {
        "HGB_G4_se4_load_weather": hgb_result["metrics"],
        "MLP_G4_se4_load_weather": mlp_result["metrics"],
        "ExtraTrees_G4_se4_load_weather": extra_result["metrics"],
    }
    direct_results = evaluate_direct_horizons(scored_rows, MODEL_PREDICTION_COLUMNS)
    weekly_summary, weekly_path_rows = evaluate_weekly_168h_paths(scored_path_rows, MODEL_PREDICTION_COLUMNS)
    comparison = compare_models(model_results, weekly_summary)
    old_new_comparison = compare_against_p0054c(model_results, weekly_summary, p0054c_summary)
    conditional_results = evaluate_conditional_regimes(scored_rows, MODEL_PREDICTION_COLUMNS)
    fairness = validate_identical_row_sets(scored_rows, MODEL_PREDICTION_COLUMNS)
    summary = {
        ...
        "weather_proxy_definition": weather_proxy_definition(),
        "p0054c_weather_contract": p0054c_summary.get("weather_contract", {}),
        ...
        "mlp_results": mlp_result["metrics"],
        "mlp_training": mlp_result["training"],
        "additional_advanced_model_results": extra_result["metrics"],
        "additional_advanced_model_training": extra_result["training"],
        "model_results": model_results,
        "model_comparison": comparison,
        "p0054c_old_vs_new_weather_comparison": old_new_comparison,
    ...
    evidence = write_p0054d_evidence(Path(evidence_dir), scored_rows, scored_path_rows, weekly_path_rows, summary)
    return P0054DResult(status=str(summary["status"]), row_counts=summary["row_counts"], evidence=evidence)
...
def load_se4_consumption_rows(feature_db: Path | str = DEFAULT_FEATURE_DB) -> list[dict[str, object]]:
    with sqlite3.connect(Path(feature_db).expanduser()) as conn:
        ...
        if not table_exists(conn, SOURCE_TABLE):
            raise RuntimeError(f"P0054D source table missing: {SOURCE_TABLE}")
        return [
...
def weather_proxy_definition() -> dict[str, object]:
    locations = [
        {"location_id": "se4_load_malmo", "name": "Malmo", "latitude": 55.6050, "longitude": 13.0038, "households_represented": 235000, "weight": 235000 / 391000},
        {"location_id": "se4_load_helsingborg", "name": "Helsingborg", "latitude": 56.0465, "longitude": 12.6945, "households_represented": 93000, "weight": 93000 / 391000},
        {"location_id": "se4_load_kristianstad", "name": "Kristianstad", "latitude": 56.0294, "longitude": 14.1567, "households_represented": 63000, "weight": 63000 / 391000},
    ]
    return {
        "area_proxy": WEATHER_AREA_PROXY,
        "source_table": "weather_locations/weather_observations/weather_area_hourly",
        "locations": locations,
        "weight_sum": sum(float(row["weight"]) for row in locations),
        "proxy_label": WEATHER_PROXY_LABEL,
    }
...
def fit_extra_trees(rows: list[dict[str, object]], features: list[str]) -> dict[str, object]:
...
def attach_model_predictions(
    ...
    extra_predictions: dict[str, list[float]],
) -> list[dict[str, object]]:
    by_split = {
        "validate": [row for row in rows if row["split"] == "validate"],
        "holdout": [row for row in rows if row["split"] == "holdout"],
    ...
    for split, split_rows in by_split.items():
        for row, hgb_pred, mlp_pred, extra_pred in zip(split_rows, hgb_predictions.get(split, []), mlp_predictions.get(split, []), extra_predictions.get(split, [])):
            row[HGB_PREDICTION_COLUMN] = hgb_pred
            row[MLP_PREDICTION_COLUMN] = mlp_pred
            row[EXTRATREES_PREDICTION_COLUMN] = extra_pred
    return rows
...
def attach_path_predictions(
    ...
    extra_pred = predict_rows(extra_result["model"], extra_result["encoder"], rows, features)  # type: ignore[arg-type]
    for row, hgb_value, mlp_value, extra_value in zip(rows, hgb_pred, mlp_pred, extra_pred):
        row[HGB_PREDICTION_COLUMN] = hgb_value
        row[MLP_PREDICTION_COLUMN] = mlp_value
        row[EXTRATREES_PREDICTION_COLUMN] = extra_value
    return rows
...
def load_p0054c_summary(path: Path = P0054C_SUMMARY_PATH) -> dict[str, object]:
    if not path.exists():
        return {}
    return json.loads(path.read_text(encoding="utf-8"))


def compare_models(model_results: dict[str, object], weekly_summary: dict[str, object]) -> dict[str, object]:
    direct = []
    for model_name, result in model_results.items():
        holdout = result["holdout"]  # type: ignore[index]
        direct.append({"model_name": model_name, "holdout_MAE": float(holdout["MAE"]), "holdout_R2": holdout["R2"]})
    best_direct = min(direct, key=lambda row: float(row["holdout_MAE"]))
    weekly = []
    weekly_mapping = {
        "HGB_G4_se4_load_weather": HGB_PREDICTION_COLUMN,
        "MLP_G4_se4_load_weather": MLP_PREDICTION_COLUMN,
        "ExtraTrees_G4_se4_load_weather": EXTRATREES_PREDICTION_COLUMN,
    }
    for model_name, column in weekly_mapping.items():
        if column in weekly_summary:
            weekly.append({"model_name": model_name, "weekly_MAE_full_168h": float(weekly_summary[column]["MAE_full_168h"])})  # type: ignore[index]
    best_weekly = min(weekly, key=lambda row: float(row["weekly_MAE_full_168h"])) if weekly else {}
    hgb_mae = next(row["holdout_MAE"] for row in direct if row["model_name"] == "HGB_G4_se4_load_weather")
    advanced = [row for row in direct if row["model_name"] != "HGB_G4_se4_load_weather"]
    best_advanced = min(advanced, key=lambda row: float(row["holdout_MAE"]))
    return {
        "direct_holdout": direct,
        "best_model_by_direct_holdout_MAE": best_direct,
        "weekly_168h": weekly,
        "best_model_by_weekly_MAE_full_168h": best_weekly,
        "best_advanced_model_by_direct_holdout_MAE": best_advanced,
        "best_advanced_minus_hgb_holdout_MAE": float(best_advanced["holdout_MAE"]) - float(hgb_mae),
        "best_advanced_relative_to_hgb_holdout_MAE_percent": (float(best_advanced["holdout_MAE"]) - float(hgb_mae)) / float(hgb_mae) * 100,
        "advanced_beats_hgb_direct_holdout": float(best_advanced["holdout_MAE"]) < float(hgb_mae),
    }


def compare_against_p0054c(model_results: dict[str, object], weekly_summary: dict[str, object], p0054c_summary: dict[str, object]) -> dict[str, object]:
    if not p0054c_summary:
        return {"available": False, "reason": "p0054c_summary_missing"}
    old_hgb = p0054c_summary["hgb_results"]["holdout"]["MAE"]  # type: ignore[index]
    old_mlp = p0054c_summary["advanced_ai_results"]["holdout"]["MAE"]  # type: ignore[index]
    new_hgb = model_results["HGB_G4_se4_load_weather"]["holdout"]["MAE"]  # type: ignore[index]
    new_mlp = model_results["MLP_G4_se4_load_weather"]["holdout"]["MAE"]  # type: ignore[index]
    old_weekly = p0054c_summary["weekly_168h_path_results"]  # type: ignore[index]
    old_hgb_weekly = old_weekly[HGB_PREDICTION_COLUMN]["MAE_full_168h"]  # type: ignore[index]
    old_mlp_weekly = old_weekly[MLP_PREDICTION_COLUMN]["MAE_full_168h"]  # type: ignore[index]
    new_hgb_weekly = weekly_summary[HGB_PREDICTION_COLUMN]["MAE_full_168h"]  # type: ignore[index]
    new_mlp_weekly = weekly_summary[MLP_PREDICTION_COLUMN]["MAE_full_168h"]  # type: ignore[index]
    old_mlp_minus_hgb = float(old_mlp) - float(old_hgb)
    new_mlp_minus_hgb = float(new_mlp) - float(new_hgb)
    old_weekly_mlp_minus_hgb = float(old_mlp_weekly) - float(old_hgb_weekly)
    new_weekly_mlp_minus_hgb = float(new_mlp_weekly) - float(new_hgb_weekly)
    return {
        "available": True,
        "comparison_type": "evidence-summary comparison; old P0054C south_connected_weather vs new P0054D se4_load_weather",
        "p0054c_weather_proxy": p0054c_summary.get("weather_contract", {}),
        "p0054d_weather_proxy": weather_proxy_definition(),
        "hgb_old_holdout_MAE": old_hgb,
        "hgb_new_holdout_MAE": new_hgb,
        "hgb_new_minus_old_holdout_MAE": float(new_hgb) - float(old_hgb),
        "hgb_new_relative_change_percent": (float(new_hgb) - float(old_hgb)) / float(old_hgb) * 100,
        "mlp_old_holdout_MAE": old_mlp,
        "mlp_new_holdout_MAE": new_mlp,
        "mlp_new_minus_old_holdout_MAE": float(new_mlp) - float(old_mlp),
        "mlp_new_relative_change_percent": (float(new_mlp) - float(old_mlp)) / float(old_mlp) * 100,
        "hgb_old_weekly_MAE_full_168h": old_hgb_weekly,
        "hgb_new_weekly_MAE_full_168h": new_hgb_weekly,
        "hgb_weekly_relative_change_percent": (float(new_hgb_weekly) - float(old_hgb_weekly)) / float(old_hgb_weekly) * 100,
        "mlp_old_weekly_MAE_full_168h": old_mlp_weekly,
        "mlp_new_weekly_MAE_full_168h": new_mlp_weekly,
        "mlp_weekly_relative_change_percent": (float(new_mlp_weekly) - float(old_mlp_weekly)) / float(old_mlp_weekly) * 100,
        "old_mlp_minus_hgb_holdout_MAE": old_mlp_minus_hgb,
        "new_mlp_minus_hgb_holdout_MAE": new_mlp_minus_hgb,
        "old_weekly_mlp_minus_hgb_MAE_full_168h": old_weekly_mlp_minus_hgb,
        "new_weekly_mlp_minus_hgb_MAE_full_168h": new_weekly_mlp_minus_hgb,
        "corrected_weather_changes_p0054c_conclusion": new_mlp_minus_hgb < 0 or new_weekly_mlp_minus_hgb < 0,
...
def select_weekly_holdout_origins(scored_rows: list[dict[str, object]]) -> dict[str, object]:
    ...
    for origin, rows in sorted(by_origin.items()):
        horizons = {int(row["horizon_h"]) for row in rows if all(row.get(column) is not None for column in MODEL_PREDICTION_COLUMNS)}
...
def evaluate_conditional_regimes(rows: list[dict[str, object]], prediction_columns: tuple[str, ...]) -> dict[str, object]:
    scored = [row for row in rows if all(row.get(column) is not None for column in MODEL_PREDICTION_COLUMNS)]
...
def write_p0054d_evidence(
    ...
    files["scored_rows_csv"] = write_csv(
        evidence_dir / "p0054d_scored_direct_rows_sample.csv",
        rows[:5000],
...
def evidence_markdowns(summary: dict[str, object]) -> dict[str, str]:
    ...
    mlp_holdout = summary["mlp_results"]["holdout"]  # type: ignore[index]
    extra_holdout = summary["additional_advanced_model_results"]["holdout"]  # type: ignore[index]
    comparison = summary["model_comparison"]  # type: ignore[index]
    old_new = summary["p0054c_old_vs_new_weather_comparison"]  # type: ignore[index]
    ...
    return {
        "CHANGELOG.md": common_header + "Built load-weighted `se4_load_weather`, reran offline SE4 consumption LABB experiment, and compared HGB, MLP and ExtraTrees. No devices, runtime writes, price, production, flow, export/import or A61 inputs were used.\n",
        "labb-label.md": common_header + "This package is LABB only. It is not G2-KANDIDAT and produces no deployable model artifact.\n",
        "se4-weather-proxy-design.md": common_header + json_block(summary["weather_proxy_definition"]),
        "se4-weather-proxy-validation.md": common_header + json_block(summary["weather_contract"]),
        ...
        "mlp-results.md": common_header + json_block({"training": summary["mlp_training"], "metrics": summary["mlp_results"]}),
        "additional-advanced-model-design.md": common_header + "Additional dependency-safe advanced model: ExtraTreesRegressor on the same G4 `se4_load_weather` row set as HGB and MLP. Deep/sequence and XGBoost/LightGBM dependencies are not locally available.\n",
        "additional-advanced-model-results.md": common_header + json_block({"training": summary["additional_advanced_model_training"], "metrics": summary["additional_advanced_model_results"]}),
        "model-comparison.md": common_header + json_block({"model_comparison": summary["model_comparison"], "p0054c_old_vs_new_weather_comparison": summary["p0054c_old_vs_new_weather_comparison"]}),
        ...
        + f"HGB holdout MAE: {hgb_holdout['MAE']:.3f} MW. MLP holdout MAE: {mlp_holdout['MAE']:.3f} MW. ExtraTrees holdout MAE: {extra_holdout['MAE']:.3f} MW. Best direct holdout model: {best_direct['model_name']}. Corrected weather comparison to P0054C is evidence-summary based: HGB new-vs-old holdout change {old_new.get('hgb_new_relative_change_percent')} percent. Weather-assisted numbers are LABB proxy results because realized archive weather is not a separate forecast model.\n",
        ...
        + json_block({"model_comparison": summary["model_comparison"], "p0054c_old_vs_new_weather_comparison": summary["p0054c_old_vs_new_weather_comparison"]}),
        "next-package-recommendation.md": common_header
        + "Recommended next package: only move to SE4-SE1 spread/flaskhals-regime LABB after deciding whether `se4_load_weather` should replace broader south weather as the default SE4 consumption proxy in future experiments.\n",
    }
...
def main() -> None:
    args = parse_args()
    result = run_p0054d_analysis(feature_db=args.feature_db, weather_db=args.weather_db, evidence_dir=args.evidence_dir)
