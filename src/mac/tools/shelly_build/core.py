...
def _manifest_label(manifest_path: str | Path) -> str:
    return Path(manifest_path).as_posix()


def _format_header(role: str, manifest_path: str | Path, sources: Iterable[str]) -> str:
    source_list = ",".join(sources)
    return "\n".join(
        [
            "// Generated by G2 Shelly build tool. Do not edit manually.",
            f"// g2_role={role}",
            f"// g2_manifest={_manifest_label(manifest_path)}",
            f"// g2_sources={source_list}",
...
def _indent_source(text: str) -> str:
    lines = text.splitlines()
    return "\n".join("" if line == "" else f"  {line}" for line in lines)


def _wrap_script(header: str, source_text: str) -> str:
    indented_source = _indent_source(source_text.rstrip("\n"))
    return "\n".join(
        [
            header.rstrip("\n"),
            "",
...
def build_script(manifest_path: str | Path, script: dict[str, Any]) -> tuple[str, str]:
    """Build one complete generated script from explicit source files."""
    ...
    header = _format_header(role, manifest_path, sources)
    return role, _wrap_script(header, "\n\n".join(parts) + "\n")
