...
from src.mac.services.spotprice_model_diagnostics import p0054c


class P0054CTests(unittest.TestCase):
    def test_lag_and_rollup_features_use_rows_strictly_before_origin(self) -> None:
        values = [float(index) for index in range(200)]
        origin_index = 168

        lags = p0054c.lag_features_at_origin(values, origin_index)
        rollups = p0054c.rolling_features_at_origin(values, origin_index)
    ...
    def test_feature_contract_excludes_forbidden_market_and_grid_inputs(self) -> None:
        review = p0054c.validate_feature_contract(p0054c.feature_group_contract())
                ...
                rows.append(
                    {
                        ...
                        "horizon_h": horizon,
                        p0054c.HGB_PREDICTION_COLUMN: float(horizon),
                        p0054c.MLP_PREDICTION_COLUMN: float(horizon),
                    }
                )

        selection = p0054c.select_weekly_holdout_origins(rows)
    ...
    def test_identical_row_set_validation_detects_matching_predictions(self) -> None:
        ...
        for split in ("validate", "holdout"):
            for horizon in p0054c.HORIZONS:
                rows.append(
                    {
                        "split": split,
                        ...
                        "horizon_h": horizon,
                        "weather_proxy_label": p0054c.WEATHER_PROXY_LABEL,
                        p0054c.HGB_PREDICTION_COLUMN: 1.0,
                        p0054c.MLP_PREDICTION_COLUMN: 2.0,
                    }
                )

        review = p0054c.validate_identical_row_sets(rows, (p0054c.HGB_PREDICTION_COLUMN, p0054c.MLP_PREDICTION_COLUMN))
