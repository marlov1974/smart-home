...
from src.mac.services.spotprice_model_diagnostics import p0054d


class P0054DTests(unittest.TestCase):
    def test_lag_and_rollup_features_use_rows_strictly_before_origin(self) -> None:
        values = [float(index) for index in range(200)]
        origin_index = 168

        lags = p0054d.lag_features_at_origin(values, origin_index)
        rollups = p0054d.rolling_features_at_origin(values, origin_index)

        self.assertEqual(lags["consumption_se4_lag_1h"], 167.0)
        self.assertEqual(lags["consumption_se4_lag_168h"], 0.0)
        self.assertNotIn(168.0, [lags["consumption_se4_lag_1h"], lags["consumption_se4_lag_168h"]])
        self.assertAlmostEqual(rollups["consumption_se4_roll_mean_24h"], sum(range(144, 168)) / 24)
        self.assertEqual(rollups["consumption_se4_roll_max_24h"], 167.0)
                ...
                rows.append(
                    {
                        ...
                        "horizon_h": horizon,
                        p0054d.HGB_PREDICTION_COLUMN: float(horizon),
                        p0054d.MLP_PREDICTION_COLUMN: float(horizon),
                        p0054d.EXTRATREES_PREDICTION_COLUMN: float(horizon),
                    }
                )

        selection = p0054d.select_weekly_holdout_origins(rows)
                ...
                rows.append(
                    {
                        ...
                        p0054d.HGB_PREDICTION_COLUMN: 1.0,
                        p0054d.MLP_PREDICTION_COLUMN: 2.0,
                        p0054d.EXTRATREES_PREDICTION_COLUMN: 3.0,
                    }
                )

        review = p0054d.validate_identical_row_sets(rows, p0054d.MODEL_PREDICTION_COLUMNS)

        self.assertTrue(review["ok"])
        self.assertTrue(review["weather_actual_proxy_labeled"])

    def test_weather_proxy_definition_weights_sum_to_one(self) -> None:
        definition = p0054d.weather_proxy_definition()

        self.assertEqual("se4_load_weather", definition["area_proxy"])
        self.assertAlmostEqual(1.0, float(definition["weight_sum"]))
