...
class ShellyBuildTests(unittest.TestCase):
    ...
    def test_build_fixture_script(self):
        ...
        self.assertEqual(["hello_v1_0_0"], [result.role for result in results])
        built = (self.build_root / "hello_v1_0_0.js").read_text(encoding="utf-8")
        self.assertTrue(built.startswith("// Generated by G2 Shelly build tool."))
        self.assertIn("// g2_role=hello_v1_0_0\n", built)
    ...
    def test_chunk_generation_writes_numeric_chunks_and_recipe(self):
        ...
        chunk_paths = sorted((self.dep_root / "ch" / "hello_v1_0_0").glob("*.js"))
        ...
        recipe = json.loads((self.dep_root / "rec" / "hello_v1_0_0.json").read_text(encoding="utf-8"))
        self.assertEqual({"v": 1, "n": len(chunk_paths)}, recipe)

    def test_chunk_reconstruction_matches_built_script(self):
        build_from_manifest(FIXTURE_MANIFEST, self.build_root, self.dep_root, max_chunk_bytes=120)

        validate_role(self.build_root, self.dep_root, "hello_v1_0_0", max_chunk_bytes=120)
        built = (self.build_root / "hello_v1_0_0.js").read_text(encoding="utf-8")
        chunks = "".join(
            path.read_text(encoding="utf-8")
            for path in sorted((self.dep_root / "ch" / "hello_v1_0_0").glob("*.js"))
        )
        self.assertEqual(built, chunks)

    def test_validation_fails_for_missing_chunk(self):
        build_from_manifest(FIXTURE_MANIFEST, self.build_root, self.dep_root, max_chunk_bytes=120)
        (self.dep_root / "ch" / "hello_v1_0_0" / "02.js").unlink()
    ...
    def test_validation_fails_for_oversize_chunk(self):
        ...
        with self.assertRaisesRegex(BuildError, "chunk exceeds max bytes"):
            validate_role(self.build_root, self.dep_root, "hello_v1_0_0", max_chunk_bytes=10)
